...
internal fun H5builder.readCompoundData(matt: AttributeMessage, h5type: H5Type) : List<*> {
    ...
    val sdataArray = this.readStructureData(state, layout, shape, members)
...
@Throws(IOException::class)
fun H5builder.readStructureData(state: OpenFileState, layout: Layout, shape : IntArray, members : List<StructureMember>): ArrayStructureData {
    val sizeBytes = Section(shape).computeSize().toInt() * layout.elemSize
    val bb = ByteBuffer.allocate(sizeBytes)
    bb.order(state.byteOrder)
    while (layout.hasNext()) {
...
@Throws(IOException::class)
fun H5builder.readData(state: OpenFileState, layout: Layout, datatype: Datatype, shape : IntArray): ArrayTyped<*> {
    val sizeBytes = Section(shape).computeSize().toInt() * layout.elemSize
    val bb = ByteBuffer.allocate(sizeBytes)
    bb.order(state.byteOrder)
    while (layout.hasNext()) {
...
// readAttributeData
internal fun H5builder.readDataAsList(dc: DataContainer): List<*> {
...
internal fun H5builder.readCompoundData(dc: DataContainer) : List<*> {
...
internal fun H5builder.readVlenData(dc: DataContainer) : List<*> {
    val shape: IntArray = dc.mds.dims
