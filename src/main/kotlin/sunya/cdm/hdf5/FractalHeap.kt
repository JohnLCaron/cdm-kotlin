   ...
   */
    ...
    inner class DHeapId internal constructor(heapId: ByteArray) {
        ...
        init {
            ...
            if (type == 0) {
                n =
                    maxHeapSize / 8 // This field's size is the minimum number of bytes necessary to encode the Maximum Heap
                // Size value
    ...
    inner class DoublingTable internal constructor(
        ...
        val blockList: MutableList<DataBlock>

        init {
            blockList = ArrayList(tableWidth * currentNumRows)
        }
    ...
    fun readDirectBlock(state: OpenFileState, heapAddress: Long, dblock: DataBlock) {
        ...
        val version: Byte = raf.readByte(state)
        val heapHeaderAddress: Long =
            h5.readOffset(state) // This is the address for the fractal heap header that this block belongs
        // to. This field is principally used for file integrity checking.
        if (heapAddress != heapHeaderAddress) throw IllegalStateException()
        dblock.extraBytes =
            5 // keep track of how much room is taken out of block size, that is, how much is left for the
        // object
        dblock.extraBytes += if (h5.isOffsetLong) 8 else 4
        var nbytes = maxHeapSize / 8
        if (maxHeapSize % 8 != 0) nbytes++
        dblock.offset = h5.readVariableSizeUnsigned(state, nbytes) // This is the offset of the block within the fractal heap's
        // address space (in bytes).
        dblock.dataPos = state.pos // state.pos; // offsets are from the start of the block
        ...
        dblock.wasRead = true
        if (debugDetail || debugFractalHeap) debugOut.println("  DirectBlock offset= " + dblock.offset + " dataPos = " + dblock.dataPos)
    }
